...
namespace ghinstaller.Modules.Commands.Options
{
    public class CommandParser
    {
        ...
        {
            foreach (var commandType in typeof(CommandParser).Assembly.GetTypes().OrderBy(x => x.Name).Where(x => x.FullName.EndsWith("Command") && x.GetCustomAttributes(typeof(BindsAttribute), false).Any()))
            {
                Info(commandType);
            }
        }
        ...
        public static string Info(Type commandType)
        {
            ...
            var bindsAttribute = (BindsAttribute) commandType.GetCustomAttributes(typeof(BindsAttribute), false).FirstOrDefault();

            if (bindsAttribute != null)
            {
                var argsType = bindsAttribute.Arguments;
                var commandAttribute = argsType.GetCustomAttributes(typeof(CommandAttribute), false).Cast<CommandAttribute>().FirstOrDefault();
        ...
        {
            var bindableAttribute = (BindsAttribute) commandType.GetCustomAttributes(typeof(BindsAttribute), false).FirstOrDefault();
            if (bindableAttribute != null)
            {
                var argumentType = bindableAttribute.Arguments;
                var commandAttribute = (CommandAttribute) argumentType.GetCustomAttributes(typeof(CommandAttribute), false).FirstOrDefault();
                if (commandAttribute != null)
                {
                    return commandAttribute.Command;
                }

                throw new Exception($"Cannot get install command, there is no CommandAttribute on {argumentType.FullName}.");
            }
